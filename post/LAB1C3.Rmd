...
> Cuidado com o efeito das escalas. Elas mudam muito no que dá para ver. 
...
ggplot(data = series, mapping = aes(x = user_rating, y = user_votes, color = series_name)) +
           ...
           facet_wrap(~series_name, scales = "free", nrow = 4,ncol = 2) 
```
...
series <- dados %>%
  ...
  summarise(seasons = max(season),
            userRatingMean = mean(user_rating)) %>%
    filter(seasons >= 15)
...
filteredSeries <- dados %>% 
    filter(series_name %in% series_400_eps$series_name) %>% 
    group_by(series_name, season) %>% 
    mutate(season_finale = season_ep == max(season_ep))

ggplot(filteredSeries, 
       mapping = aes(x = series_ep, 
                     y = user_rating, 
                     color = season_finale)) +
...
```{r}
ggplot(filteredSeries, 
       mapping = aes(x = season_finale, 
                     y = user_rating, 
                     color = season_finale)) +
    geom_jitter(alpha = .7, size = .6, width = .2) +
    facet_wrap(~series_name, scales = "free_x", nrow = 3, ncol = 2) 
```
...
> Repara que aqui você respondeu sobre episódios, e não sobre séries... 
